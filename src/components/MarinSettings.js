  ...
  render() {
    if (this.state.renderMachine) {
      return (
        <Machine
          selection="marin"
          kysymykset={this.state.kysymykset}
          selected_categories={this.state.categories}
          howMany={this.state.number}
        />
      );
    }
    return (
      <div>
        <h1>Asetukset</h1>
        <p>
          Oletuksena sinulle arvotaan 20 kysymystä. Halutessasi voit valita
          kysymysten määrän ja sinua kiinnostavat kategoriat.
        </p>
        <p>
          Vaalikausikoneissa puoleita ja ryhmiä käsitellään aina kokonaisuutena.
          Tulosten kannalta tämä tarkoittaa sitä, että puolueen katsotaan
          kussakin kysymyksessä edustavan 100 prosenttisesti enemmistönsä
          linjaa.
        </p>
        <Accordion style={{ paddingTop: "1em", paddingBottom: "1em" }}>
          ...
          <Accordion.Title
            active={this.state.howMany}
            onClick={this.showHowMany}
          >
            <Icon name="dropdown" />
          ...
          >
            <div>
              {this.state.kysymykset?.length &&
                [10, 20, this.state.kysymykset.length].map((number) => (
                  <Checkbox
                    key={number}
