         ...
         {this.state.order.map(x =>
               <Table.Row key={x.name}>
                 ...
                 <Modal 
                  ...
                  >
                     <Modal.Header>
                      {this.finnishParties(x.name)} <Icon style={{right: "5%", position: 'absolute'}}name="close" onClick={() => this.setState({show: false})}/>
                      </Modal.Header>
                    <Modal.Content>
                    {this.props.kayttaja.kysymykset.map(question =>
                                  ...
                                  <Table.Body>
                                      <Table.Row >
                                        ...
                                        <Table.Cell style={{width: '20%', background: this.color(question.puolueet.find(party => party.nimi == x.name).kanta)}}>
                                        {this.result(question.puolueet.find(party => party.nimi == x.name))}
                                        </Table.Cell>
                                        <Table.Cell style={{width: '20%', background: this.color(question.user)}}>
                                          {"Sinä vastasit " + question.user} 
                                        </Table.Cell>
                                      </Table.Row>
                                  </Table.Body>
                                </Table>
