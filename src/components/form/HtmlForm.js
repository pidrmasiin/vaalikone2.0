...
class HtmlForm extends React.Component {
  ...
  componentWillMount = async () => {
    if (!this.props.ylenKysymykset.kysymykset) {
  ...
  handlePuolueet = () => {
    ...
    const puolueet = [];
    if (
      this.props.html.puolueet !== "" &&
      doc.getElementsByTagName("TBODY")[0]
    ) {
      ...
      for (let i = 1; i < rows.length; i = i + 1) {
        ...
        puolue.kanta = Object.keys(puolue).reduce((a, b) =>
          puolue[a] > puolue[b] ? a : b
        );
  ...
  addKatet = () => {
    ...
    this.props.kategoriat.map((k) =>
      document.getElementById(k.nimi).checked ? kategoriat.push(k) : null
    );
  ...
  checkExist = (e) => {
    if (
      e.target.value &&
      this.props.kysymykset.filter(
        (k) =>
          this.validateTunniste(k.tunniste) ==
  ...
  render() {
    ...
    /*eslint-disable */
    const values = this.props.ylenKysymykset.kysymykset.map(
      (x) => (x = { text: x, value: x })
    );
    const yleValues2019 = yle2019Questions.map(
      (x) => (x = { text: x, value: x })
    );
        ...
        <form onSubmit={this.onSubmit} id="htmlform">
          ...
          <div className="form-group">
            ...
            <Form.Input
              ...
              placeholder="HE 123/2001 vp"
              name="tunniste"
              onChange={(e) => this.checkExist(e)}
            />
          </div>
          <TextArea
            label="Kirjoita alle äänestyksen kohteen oleva kysymys"
            placeholder="kysymys"
            name="kysymys"
          />
          <FormInput label="Tapahtuma vuosi" placeholder="2018" name="vuosi" />
          <FormInput
            label="Linkki edukunnan sivuille"
            placeholder="url"
            name="url"
          />
          ...
          {this.state.exist ? (
            ...
            <p>
              <Button positive type="submit" className="fluid ui button">
                create
              </Button>
            </p>
          )}
