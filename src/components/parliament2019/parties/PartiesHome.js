...
class Home extends React.Component {
  ...
  parseQuestion = (q) => {
    let slicedQ =
      q.length > 50 && window.innerWidth < 700 ? q.slice(0, 50) + "..." : q;
    return slicedQ;
  };
  ...
  singlePartyOpinion = (members, opinon, party) => {
    if (opinon !== "jaa" && opinon !== "ei") {
      return members
        .filter((mem) => mem.puolue === party)
        .filter(
          (x) =>
            x.kanta.toLowerCase() == "tyhja" ||
            x.kanta.toLowerCase() == "poissa"
        ).length;
    }
    return members
      .filter((mem) => mem.puolue === party)
      .filter((x) => x.kanta.toLowerCase() == opinon).length;
  };

  singleYleOpinion = (party, opinon, question) => {
    return party.filter(
      (member) => yleQuestions.opinonsHelper(member[question]) == opinon
    ).length;
  };

  render() {
    const dropParties = Object.keys(this.props.yle2019.parties).map((party) => {
      return {
        text: party,
        value: party,
      };
    ...
    const questions = this.props.questions
      ...
      .map((q) => (q = { text: q.kysymys, value: q.id }));
    ...
    let members = [];
    if (this.state.qid) {
      members = this.props.questions
        .find((q) => q.id === this.state.qid)
        .edustajat.map((x) => ({
          nimi: x.nimi.split("/")[0],
    ...
    return (
      <div>
        <div style={{ marginLeft: "10%", marginRight: "10%" }}>
          <h1
            style={{ textAlign: "center", margin: "1.2em", marginTop: "0.5em" }}
          >
            Puolueet
          </h1>
          <Parties />
          ...
          <p style={{ textAlign: "justify" }}>
            Voit valita kaksi puoluetta ja vertailla niiden
            äänestyskäyttäytymistä ja vastauksia vuoden 2019 Ylen
            vaalikonekysymykseen. Vaalikonevastauksissa on mukana myös varalla
            olijat, ja melko samaa mieltä on tulkittu jaaksi ja melko eri mieltä
            eiksi.
          </p>
          ...
          <Dropdown
            ...
            type="text"
            name="yleQuestion"
            placeholder="Valitse vaalikonekysymys"
            onChange={this.handleChange.bind(this)}
            fluid
            selection
            options={yleQue}
            ...
            {this.state.qid && (
              <Grid.Row>
                <Grid.Column width={16}>
                  <hr />
                  {yleQuestions.parseQuestionOut(
                    questions.find((x) => x.value == this.state.qid).text
                  )}
                </Grid.Column>
              </Grid.Row>
            )}
                  ...
                  <div className="parties-pie">
                    <Pie
                      ...
                      data={{
                        labels: [
                          "Jaa (" +
                            this.singleYleOpinion(
                              this.props.yle2019.parties[this.state.party],
                              "jaa",
                              this.state.yleQuestion
                            ) +
                            ")",
                    ...
                    <Pie
                      ...
                      data={{
                        labels: [
                          "Jaa (" +
                            this.singleYleOpinion(
                              this.props.yle2019.parties[this.state.party2],
                              "jaa",
                              this.state.yleQuestion
                            ) +
                            ")",
            ...
            <Grid.Row>
              <Grid.Column width={8}>
                <Table celled>
                  ...
                  <Table.Body>
                    {members
                      .filter((mem) => mem.puolue === this.state.party)
                      .map((x) => (
                        <Table.Row>
                          ...
                          <Table.Cell>
                            {this.state.yleQuestion &&
                              this.props.yle2019.parties[this.state.party].find(
                                (mem) => mem.sukunimi + mem.etunimi == x.nimi
                              ) &&
                              yleQuestions.opinonsHelper(
                                this.props.yle2019.parties[
                                  this.state.party
                                ].find(
                                  (mem) => mem.sukunimi + mem.etunimi == x.nimi
                                )[this.state.yleQuestion]
              ...
              <Grid.Column width={8}>
                <Table celled>
                  ...
                  <Table.Body>
                    {members
                      .filter((mem) => mem.puolue === this.state.party2)
                      .map((x) => (
                        <Table.Row>
                          <Table.Cell>{x.nimi}</Table.Cell>
                          <Table.Cell>{x.kanta}</Table.Cell>
